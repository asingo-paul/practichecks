...
export default function HirerEditInternshipPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect back to internship view after success
      setTimeout(() => {
        router.push(`/hirer/my-internships/${internshipId}`);
      }, 2000);
    } catch (err) {
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        {isLoadingData ? (
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              <Link
                href={`/hirer/my-internships/${internshipId}`}
                className="p-2 hover:bg-gray-100 rounded-lg"
              >
                <ArrowLeftIcon className="h-5 w-5 text-gray-600" />
              </Link>
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Edit Internship</h1>
                <p className="text-gray-600">Update your internship posting</p>
              </div>
            </div>

            {success && (
              <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg">
                ✓ Internship updated successfully! Redirecting to internship details...
              </div>
            )}
